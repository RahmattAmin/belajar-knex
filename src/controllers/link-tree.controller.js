...
async function linkTreeGetLinks(_, res) {
  const link = await models.findAll();
  const data = {
    link,
    total: link.length,
  };
...
function linkTreeAddNewLink(req, res) {
  ...
  if (!body.name) return res.status(401).json(responseError("name is required"));
  if (!body.link) return res.status(401).json(responseError("link is required"));
...
function linkTreeEditLinkByName(req, res) {
  ...
  return res.status(200).json(responseOk(`Success edit ${name} to ${data.name} link`, data));
}
